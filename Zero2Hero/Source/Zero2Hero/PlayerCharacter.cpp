...
void APlayerCharacter::BeginPlay()
{
	...
	GrapplingHook->AttachToComponent(GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale);
	...
	for (int i = 0; i < RangedWeapons.Num(); i++)
	{
		if (RangedWeapons[i] != nullptr)
		{
			...
			allRangedWeapons[i]->AttachToComponent(GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale);
